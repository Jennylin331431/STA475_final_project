...
head(data_mutated)
unique(data_mutated$SMOKER)
sum(is.na(data_mutated$SMOKER))
sum(is.na(data_mutated$MONTHS))
sum(is.na(data_mutated$STATUS))
sum(is.na(data_mutated$SEX))

count(data_mutated)
```

```{r}
model_cox <- coxph(Surv(MONTHS, STATUS) ~ factor(SEX) + MSI_SCORE + MUTATION_COUNT + FRACTION_GENOME_ALTERED + factor(SMOKER),
                  data=data_mutated)
summary(model_cox)
```

```{r}
# Create a frequency table of the 'SMOKER' column
mode_smoker <- names(sort(table(data_mutated$SMOKER), decreasing = TRUE))[1]
