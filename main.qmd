...
The active smoker seem to have a lower survival probability at a given time comparing to the two other groups. Female seem to have a lower survival probability at a given time compare to male.
...
fig.res1 <- tibble(AGE_AT_DX = data_mutated$AGE_AT_DX, 
                    residuals = cox.resid) %>% 
...
fig.res3 <- tibble(SYSTEMIC_TREATMENT = data_mutated$SYSTEMIC_TREATMENT, 
                    residuals = cox.resid) %>% 
  ggplot(aes(x = SYSTEMIC_TREATMENT, y = residuals)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_boxplot()
...
### obtain performance amongst different specimen stages amongst chemotherapy patients with all other median valued covariates
...
sf_cox <- survfit(model_cox_3_strata, newdata = newdata, data = data_mutated)
...
```
### obtain performance for different systemic treatments on patients with median values of other covariates 
