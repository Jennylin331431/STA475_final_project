...
data_mutated %>% 
  ...
  scale_color_manual(values = c("lightblue", "black")) +
  labs(
    x = "Log of Fraction Genome Altered",  # Updated x-axis label
    y = "Log of Survival Time (Months)"  # Updated y-axis label
  ) +
  theme_minimal()
...
data_mutated %>% 
  ...
  labs(x = "Age at Diagnosis", 
       y = "Log of Survival Time (Months)", 
       title = "Distribution of log(Survival Months) over Age at Diagnosis") +
  theme_minimal()
...
ggsurvplot(km_fit, data = data_mutated,
           ggtheme = theme_minimal(),
           title = "KM Curve by Mutation Count Quartiles",
...
ggsurvplot(km_fit, data = data_mutated,
           ggtheme = theme_minimal(),
           title = "KM Curve by Fraction Genome Altered Quartiles",
 ...
 font.x = 16, 
 font.y = 16, 
                     )# Break MSI_SCORE into quartiles
...
ggsurvplot(km_fit, data = data_mutated,
           ggtheme = theme_minimal(),
           title = "KM Curve by MSI Score Quartiles",
 ...
 font.y = 16, 
 theme = theme(
               plot.title = element_text(hjust = 5, vjust = 10),  # Center the title horizontally, 
...
```{r}
# 6. Age at Diagnosis
data_mutated$AGE_DX_QUARTILE <- cut(
  data_mutated$AGE_AT_DX, 
  breaks = quantile(data_mutated$AGE_AT_DX, probs = seq(0, 1, 0.25), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Q1 (Lowest)", "Q2", "Q3", "Q4 (Highest)")
...
# Fit Kaplan-Meier model by quartile
km_fit <- survfit(Surv(MONTHS, STATUS) ~ AGE_DX_QUARTILE, data = data_mutated)

# Plot Kaplan-Meier curves
ggsurvplot(km_fit, data = data_mutated,
           ggtheme = theme_minimal(),
           title = "KM Curve by Age at Diagnosis Quartiles",
...
model1 <- survreg(Surv(MONTHS, STATUS) ~ SEX + MSI_SCORE + MUTATION_COUNT + FRACTION_GENOME_ALTERED + SMOKER + AGE_AT_DX,
...
# Model 2
model2 <- survreg(Surv(MONTHS, STATUS) ~ SEX + MSI_SCORE + MUTATION_COUNT + FRACTION_GENOME_ALTERED + AGE_AT_DX,
                  data = data_mutated, dist = "lognormal")
...
# Model 3
model3 <- survreg(Surv(MONTHS, STATUS) ~ SEX + MUTATION_COUNT + FRACTION_GENOME_ALTERED + AGE_AT_DX,
                  data = data_mutated, dist = "lognormal")
...
```{r}
# Model 4
model4 <- survreg(Surv(MONTHS, STATUS) ~ MUTATION_COUNT + FRACTION_GENOME_ALTERED + AGE_AT_DX,
                  data = data_mutated, dist = "lognormal")
...
```{r}
# Model 5
model5 <- survreg(Surv(MONTHS, STATUS) ~ MUTATION_COUNT + AGE_AT_DX,
                  data = data_mutated, dist = "lognormal")
...
lr_summary_aft <- 
  tibble(model=c(1, 2, 3, 4, 5),
         ...
         df_for_comp = c(NA, 9-7, 7-6, 6-5, 5-4),
         pvalue = c(NA,
...
model_cox_1 <- coxph(Surv(MONTHS, STATUS) ~ MUTATION_COUNT + FRACTION_GENOME_ALTERED +
                                    SEX + MSI_SCORE + SMOKER + AGE_AT_DX,
                  data= data_mutated)
summary(model_cox_1)
```
...
# Remove Sex 
model_cox_4 <- coxph(Surv(MONTHS, STATUS) ~ MUTATION_COUNT + FRACTION_GENOME_ALTERED + AGE_AT_DX,
                  data=data_mutated)
summary(model_cox_4)
```
...
lr_summary_cox <- 
  tibble(model=c(1, 2, 3, 4),
         ...
         df_for_comp = c(NA, 7-6, 6-4, 4-3),
         pvalue = c(NA,
                    pchisq(2*(model_cox_1$loglik[2] - model_cox_2$loglik[2]),
                           df= 7-6, lower.tail=FALSE),
                    ...
                    pchisq(2*(model_cox_3$loglik[2] - model_cox_4$loglik[2]),
                           df= 4-3, lower.tail=FALSE)))

kable(lr_summary_cox)
```

Similar approaches were taken to compare the Cox PH models as the AFT models. Based on the Likelihood Ratio Tests, it seems that Model 4 seems to be best to model our data. Our final model would then include the covariates of mutation count, fraction genome altered, sex, and smoker status.
...
# Plot residuals over covariate Fraction Genome Altered
fig.res8 <- tibble(AGE_AT_DX = data_mutated$AGE_AT_DX, 
                    residuals = cox.resid) %>% 
  ggplot(aes(x = AGE_AT_DX, y = residuals)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_point()
...
ggarrange(fig.res1, fig.res2, fig.res3,
          ...
          fig.res7, fig.res8, ncol = 3, nrow = 3)
