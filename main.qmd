...
```{r, eval=FALSE}
# load the data
file_path <- "paired_bladder_2022_clinical_data_test2.tsv"

# Read the JSON data into R
data_temp <- fromJSON(file_path)

# Convert the resulting list into a data frame, if needed
data_temp <- as.data.frame(data_temp)

# Remove the studyId column
data_temp <- data_temp %>% select(-studyId)
```

```{r, warning=FALSE, message=FALSE}
# Complete dataset with 1659 samples
data <- read_tsv("paired_bladder_2022_clinical_data_full.tsv",
...
data_mutated <- data %>%
  ...
  select(patientId, MONTHS, STATUS, SEX, MSI_SCORE, MUTATION_COUNT, FRACTION_GENOME_ALTERED, SMOKER)
...
```
(Weibull fit the data best as the lines are the most parallel.) Lognormal seems to fit best.
...
# Only keep MSI, Mutation Count, FGA
model2 <- survreg(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT
                  + FRACTION_GENOME_ALTERED + SMOKER,
                  data = data_mutated, dist = "lognormal")
...
# Try to remove Smoker?
model3 <- survreg(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT
...
loglik_model1 <- summary(model1)$loglik[2]
loglik_model2 <- summary(model2)$loglik[2]

lr_stat <- 2 * (loglik_model1 - loglik_model2)
p_value_lr <- pchisq(lr_stat, df = 8 - 7, lower.tail = FALSE)
cat("LR Test Statistic:", lr_stat, "\nP-value:", p_value_lr, "\n")

```
$H_0:$ Model 2 is better then Model 1 vs $H_A:$ Model 1 is better than Model 2.

At 5% significance, we have no evidence against the null hypothesis that Model 2 is better than Model 1. So we should use model 2. 

```{r}
# Likelihood Ratio Test between Model 2 and Model 3
loglik_model2 <- summary(model2)$loglik[2]
loglik_model3 <- summary(model3)$loglik[2]

lr_stat <- 2 * (loglik_model2 - loglik_model3)
p_value_lr <- pchisq(lr_stat, df = 7 - 5, lower.tail = FALSE)
cat("LR Test Statistic:", lr_stat, "\nP-value:", p_value_lr, "\n")

```
$H_0:$ Model 3 is better than model 2 vs $H_A:$ Model 2 is better than Model 3.

At 5% significance, we have strong evidence against the null hypothesis that Model 3 is better than Model 2. So we should use model 2.
...
alpha <- beta_hat[1] +
  beta_hat[2] * data_mutated$MSI_SCORE +
  beta_hat[3] * data_mutated$MUTATION_COUNT +
  beta_hat[4] * data_mutated$FRACTION_GENOME_ALTERED +
  beta_hat[5] * c(data_mutated$SMOKER == "Former") +
  beta_hat[5] * c(data_mutated$SMOKER == "Never")

# Full Model Template
# alpha <- beta_hat[1] +
#   beta_hat[2] * c(data_mutated$SEX == "Male") +
#   beta_hat[3] * data_mutated$MSI_SCORE +
#   beta_hat[4] * data_mutated$MUTATION_COUNT +
#   beta_hat[5] * data_mutated$FRACTION_GENOME_ALTERED +
#   beta_hat[5] * c(data_mutated$SMOKER == "Former")+
#   beta_hat[6] * c(data_mutated$SMOKER == "Never")
...
```{r}
# Try log-logistic?
model4 <- survreg(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT
                  + FRACTION_GENOME_ALTERED + SMOKER,
                  data = data_mutated, dist = "loglogistic")
...
```{r}
# Try Weibull?
model5 <- survreg(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT
                  + FRACTION_GENOME_ALTERED + SMOKER,
                  data = data_mutated, dist = "exponential")
...
We still observe consistent significance of variables for either Weibull or log-logistic model.

Residual Analysis for Model 2 (Log-logistic)

```{r}
# Extract parameter estimates from model2 (Log-logistic Model)
...
```{r}
# Extract parameter estimates from model2 (Weibull Model)
beta_hat <- model5$coef
alpha <- beta_hat[1] +
  beta_hat[2] * data_mutated$MSI_SCORE +
  beta_hat[3] * data_mutated$MUTATION_COUNT +
  beta_hat[4] * data_mutated$FRACTION_GENOME_ALTERED +
...
model_cox_2 <- coxph(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT + FRACTION_GENOME_ALTERED + SMOKER,
                  data=data_mutated)
summary(model_cox_2)
```
...
model_cox_3 <- coxph(Surv(MONTHS, STATUS) ~ MUTATION_COUNT + FRACTION_GENOME_ALTERED + SMOKER,
                  data=data_mutated)
summary(model_cox_3)
```
...
At 5% significance, we have some evidence against the null hypothesis that Model 2 is as good as Model 3. Hence, let's settle on Model 2.  

**Therefore, we should use our second model, which includes sex and smoker as our predictors. **
...
newdata <- data.frame(MSI_SCORE = median(data_mutated$MSI_SCORE),
...
```{r}
 # Assuming `model_cox_2` is your Cox proportional hazards model
...
# PH Assumption Check
km_former <- survfit(Surv(MONTHS, STATUS) ~ 1, data=data_mutated,
              subset = (SMOKER =="Former"))
km_active <- survfit(Surv(MONTHS, STATUS) ~ 1, data=data_mutated,
              subset = (SMOKER=="Active"))
km_never <- survfit(Surv(MONTHS, STATUS) ~ 1, data=data_mutated,
              subset = (SMOKER=="Never"))

tibble(time = c(km_former$time, km_active$time, km_never$time),
       surv = c(km_former$surv, km_active$surv, km_never$surv),
       group = c(rep("Former", length(km_former$time)), 
                 rep("Active", length(km_active$time)),
                 rep("Never", length(km_never$time)))
       ) %>% 
  ...
  labs(x = "Log Survival Time",
       ...
       colour = "Treatment Groups",
       )
```
Note that the PH assumption seems to be violated here. Stratify based on Smoker then.

```{r}
# Stratify Model 2 on Smoker
model_cox_4 <- coxph(Surv(MONTHS, STATUS) ~ MSI_SCORE + MUTATION_COUNT + FRACTION_GENOME_ALTERED + strata(SMOKER),
                  data=data_mutated)
...
fig.res3 <- tibble(number = data_mutated$FRACTION_GENOME_ALTERED, 
                    residuals = cox.resid) %>% 
  ggplot(aes(x = number, y = residuals)) +
  geom_hline(yintercept = 0, linetype = 2) +
  geom_point()
...
fig.res5 <- tibble(risk.score = predict(model_cox_2), 
                    residuals = cox.resid) %>% 
...
ggarrange(fig.res1, fig.res2, fig.res3,
          fig.res4, fig.res5, fig.res6, ncol = 3, nrow = 2)
